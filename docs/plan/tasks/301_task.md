...
12. **[T4-2] カテゴリアイコン表示機能実装**
    - 概要: カテゴリ名とアイコン画像をマッピングし、記事一覧等で表示する。
    - 完了条件:
        - [ ] カテゴリ名とアイコンファイルパスのマッピング定義 (`/lib/icons/icon-map.ts` 等) が作成されている。
        - [ ] `public/images/icons/` に必要なアイコンファイルが配置されている (typescript.png, nextjs.png, ruby.png など)。
        - [ ] カテゴリ名から対応するアイコンファイルパスを取得するユーティリティ関数が実装されている (任意)。
        - [ ] 記事カードコンポーネント (`/features/post/_components/PostCard.tsx`) が修正され、カテゴリ名の隣などにアイコンが表示されるようになっている。
        - [ ] 記事一覧ページ (`/app/(routes)/blog/page.tsx`) でアイコンが表示される。
        - [ ] カテゴリ別記事一覧ページ (`/app/(routes)/blog/categories/[category]/page.tsx`) でアイコンが表示される (任意、ヘッダー等で表示)。
    - 動作確認:
        - [ ] `npm run dev` で記事一覧ページを開き、各記事のカテゴリに対応するアイコンが表示されること。
        - [ ] カテゴリにアイコンがマッピングされていない場合の代替表示（例: デフォルトアイコン or 表示なし）が適切であること。