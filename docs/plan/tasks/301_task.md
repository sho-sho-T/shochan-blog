...
14. **[T4-4] ホーム画面への最新記事表示**
    ...
    - 完了条件:
        - [ ] `/app/(routes)/page.tsx` で `getAllPosts` ユーティリティを使用し、公開済み (`status: 'published'`) の記事を公開日 (`publishedAt`) の降順で取得する。
        - [ ] 取得した記事の中から最新の数件 (例: 3件) を選択する。
        - [ ] `PostCard` コンポーネントを使用して、選択した記事の一覧を表示する。
        - [ ] セクションのタイトル (例: "新着記事", "Recent Posts") を表示する。
        - [ ] 全記事一覧ページ (`/blog`) へのリンク (例: "もっと見る", "View all posts") を表示する。
    - 動作確認:
        - [ ] `npm run dev` でホーム画面 (`/`) にアクセスし、最新記事のセクションが表示され、記事カードが正しく表示・リンクされていることを確認。
        - [ ] "もっと見る" リンクが `/blog` に正しく遷移することを確認。
        - [ ] 記事が存在しない場合でもエラーなく表示されること。

15. **[T4-5] ヘッダー検索機能実装**
    - 概要: ヘッダーにカテゴリおよびタグによる記事検索機能を追加する。shadcn-ui の `Command` や `Dialog` を使用して UI を構築する。
    - 完了条件:
        - [ ] `components/layout/header.tsx` に検索アイコンボタン (`MagnifyingGlassIcon` 等) を追加する。
        - [ ] ボタンクリック時に shadcn-ui の `Dialog` または `CommandDialog` が開く。
        - [ ] `Dialog` 内に `Command` コンポーネントを配置し、検索入力フィールドとカテゴリ/タグのリストを表示する。
        - [ ] `getAllCategories` および `getAllTags` ユーティリティを使用して、存在するカテゴリとタグのリストを取得し、`CommandList` 内に表示する。
        - [ ] ユーザーがリストからカテゴリまたはタグを選択すると、対応する一覧ページ (`/blog/categories/[category]` または `/blog/tags/[tag]`) に遷移する。
        - [ ] (任意) 検索入力フィールドでカテゴリ/タグ名をフィルタリングできる。
    - 動作確認:
        - [ ] `npm run dev` でヘッダーの検索ボタンをクリックし、検索ダイアログが表示されること。
        - [ ] ダイアログ内にカテゴリとタグのリストが表示されること。
        - [ ] リストから項目を選択すると、対応するカテゴリ/タグのページに遷移すること。
        - [ ] ダイアログが正しく閉じられること。